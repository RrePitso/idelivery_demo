...
import { sendWhatsAppSignupNotification, formatSAFullPhone } from '../utils/whatsapp';
...
const SignUp: React.FC = () => {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await sendWhatsAppSignupNotification({
        destination: formattedPhone,
        userName: formData.fullName,
        templateParams: [
          "Alice Campus Main Gate",
          "Alice Town Center"
        ]
      });
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-md text-center px-4">
        ...
        <p className="mt-2 text-sm text-gray-600 italic">
          Already a driver?{' '}
          <Link to="/drivers/signin" className="font-bold text-[#F58220] hover:text-orange-600 transition-colors">
            Sign in
          </Link>
        </p>
      </div>
