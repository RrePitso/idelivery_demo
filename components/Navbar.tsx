  ...
  return (
    <nav className="bg-white border-b border-gray-100 px-4 py-3 sticky top-0 z-50 shadow-sm">
      <div className="max-w-7xl mx-auto flex justify-between items-center">
        <Link to="/drivers" className="flex items-center space-x-2 transition-transform active:scale-95">
        ...
        <div className="flex items-center space-x-4 text-xs font-black uppercase tracking-widest">
          <Link to="/drivers/simulator" className="text-gray-400 hover:text-green-600 transition-colors">Bot Test</Link>
          {!state.user ? (
            <>
              <Link to="/drivers/signin" className="text-gray-600 hover:text-[#F58220] transition-colors">Sign In</Link>
              <Link to="/drivers/signup" className="bg-[#50B848] text-white px-4 py-2 rounded-full hover:bg-[#45a03f] transition-all shadow-lg shadow-green-100">Join</Link>
            </>
          ) : (
            <>
